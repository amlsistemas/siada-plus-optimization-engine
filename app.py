...
def numero_a_romano(n):
    """Convierte número entero a romano (1-10)."""
    romanos = {
        1: "I", 2: "II", 3: "III", 4: "IV", 5: "V",
        6: "VI", 7: "VII", 8: "VIII", 9: "IX", 10: "X"
    }
    return romanos.get(int(n), str(n)) if n else "N/A"
            ...
            for g in range(num_grupos):
                for i in range(num_instructores):
                    if solver.Value(asignacion[(i, d, g)]) == 1:
                        ...
                        # Corregir Trimestre
                        # ============================================================
                        trimestre_raw = grupo_info.get('Trimestre', '1')  # Del grupo
                        # Sobreescribir con currículo si está disponible
                        sesiones_trimestre = curriculo_por_trimestre.get(trimestre_raw, curriculo_sessions)
                        if sesiones_trimestre:
                            # Usa el trimestre del currículo para la sesión actual
                            curr_idx = (d * num_grupos + g) % len(sesiones_trimestre)
                            curr_info = sesiones_trimestre[curr_idx]
                            trimestre_raw = curr_info.get('trimestre', trimestre_raw)
                        try:
                            trimestre_num = int(float(str(trimestre_raw).strip()))
                            trimestre_display = numero_a_romano(trimestre_num)
                        except (ValueError, TypeError):
                            trimestre_display = str(trimestre_raw).upper()
                        ...
                        resultados.append({
                            ...
                            "Trimestre": trimestre_display,  # Aquí el valor corregido
    ...
    if all([uploaded_grupos, uploaded_instructores, uploaded_ambientes, uploaded_curriculo]):
        ...
        if not horario_df.empty:
            ...
            columnas_ordenadas = [
                "Fecha", "Día", "Jornada", "Ficha", "Programa", "Trimestre", "Hora Inicio", "Hora Fin",
                "Municipio", "Asignatura", "Competencia", "Resultados de Aprendizaje",
                "Instructor", "Jornada del Instructor", "Ambiente", "Estado", "Notas", "Festivo", "Exclusiones del Instructor"
            ]
